...
class SystemSettings extends DBConnection{
	...
	function update_settings_info(){
		...
		if(isset($_FILES['logmas']) && $_FILES['logmas']['tmp_name'] != ''){
			$fname = 'uploads/'.strtotime(date('y-m-d H:i')).'_'.$_FILES['logmas']['name'];
			$move = move_uploaded_file($_FILES['logmas']['tmp_name'],'../'. $fname);
			if(isset($_SESSION['system_info']['logmas'])){
				$qry = $this->conn->query("UPDATE system_info set meta_value = '{$fname}' where meta_field = 'logmas' ");
				if(is_file('../'.$_SESSION['system_info']['logmas'])) unlink('../'.$_SESSION['system_info']['logmas']);
			}else{
				$qry = $this->conn->query("INSERT into system_info set meta_value = '{$fname}',meta_field = 'logmas' ");
			}
		}
